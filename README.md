...
**机主识别细节**：录入时正对屏幕缓慢转头，4 秒采 8 个特征印迹样本（质量门控）。样本经 NSSecureCoding 归档存 Keychain（ThisDeviceOnly），匹配走 `computeDistance` 取最近样本距离。调参：打开调试浮层（菜单栏 → 调试浮层）看自己脸上的 `d=` 实时值与 OWNER/STRANGER 标签，再把"识别严格度"滑块调到「自己 d 值 + 0.15」左右。
