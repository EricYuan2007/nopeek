...
public enum IntruderAssessor {
    ...
    public static func assess(faces: [FaceInfo], config: DetectionConfig) -> Assessment {
        ...
        guard let owner = sorted.first else {
            return Assessment(owner: nil, intruders: [], faceCount: 0)
        }
        let intruders = sorted.dropFirst().filter { isIntruder($0, config: config) }
        return Assessment(owner: owner, intruders: intruders, faceCount: sorted.count)
    }
